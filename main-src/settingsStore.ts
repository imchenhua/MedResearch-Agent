...
import { getCachedMedResearchDataDir, resolveMedResearchDataDir } from './dataDir.js';
...
export type ShellUiSettings = {
	...
	themePresetId?: 'medresearch' | 'cursor' | 'graphite' | 'forest' | 'sunset' | 'custom';
	accentColor?: string;
 ...
 * 在 `<asyncData>/plugins/<name>/` 或 `<workspace>/.medresearch/plugins/<name>/` 下放置 `.lsp.json` 或 `plugin.json#lspServers`。
...
export function initSettingsStore(userData: string): void {
	const dir = resolveMedResearchDataDir(userData);
...
export function getDefaultUserPluginsRoot(): string {
	const root = path.join(getCachedMedResearchDataDir(), 'plugins');
