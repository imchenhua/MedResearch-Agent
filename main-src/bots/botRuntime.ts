...
function buildBotBrowserAttachmentDir(session: BotSessionState): string {
	if (session.workspaceRoot) {
		return path.join(session.workspaceRoot, '.medresearch', 'bot-attachments');
	}
	return path.join(app.getPath('userData'), 'medresearch', 'bot-attachments');
}
...
const BOT_TOOL_DEFS: AgentToolDef[] = [
	...
	},
	{
		...
		description: 'Switch the active MedResearch Agent workspace for this conversation. Use a value returned by get_async_session.availableWorkspaces.',
		parameters: {
			type: 'object',
			properties: {
	...
	{
		name: 'switch_model',
		description: 'Switch the active MedResearch Agent model for this conversation. Use a model id returned by get_async_session.availableModels.',
		parameters: {
			type: 'object',
			properties: {
	...
	},
	{
		...
		description: 'Start a fresh internal MedResearch Agent worker thread for the current workspace. Use this before run_async_task when you intentionally want a clean worker context.',
		parameters: {
			type: 'object',
			properties: {
	...
	},
	{
		name: 'run_async_task',
		description:
			'Launch an internal MedResearch Agent worker session when you intentionally want a detached worker or specialist workflow. Prefer using your own tools directly unless you explicitly want a worker session. Choose mode automatically: agent for direct execution, ask for lightweight Q&A, plan for planning-only work, team for delegated multi-role work.',
		parameters: {
			type: 'object',
			properties: {
		...
		parameters: {
			...
			properties: {
				message: {
					type: 'string',
					description:
						'Optional short user-facing instruction. If omitted, MedResearch Agent sends a default "scan the QR and reply when done" message.',
				},
				tab_id: {
					type: 'string',
...
export function buildBotOrchestratorPrompt(
	...
	const lines = [
		language === 'en'
			? 'You are the MedResearch Agent global leader bot. You directly manage the MedResearch Agent app, its tools, and its worker sessions.'
			: '你是 MedResearch Agent 的全局 Leader Bot。你直接管理整个 MedResearch Agent 应用、它的工具能力以及内部 worker 会话。',
		language === 'en'
