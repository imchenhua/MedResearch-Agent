...
export function providerSupportsAnthropicNativeDefer(baseURL?: string): boolean {
	if (isEnvTruthy(process.env.MEDRESEARCH_ANTHROPIC_NATIVE_DEFER)) {
		return true;
	}
	if (isEnvFalsy(process.env.MEDRESEARCH_ANTHROPIC_NATIVE_DEFER)) {
		return false;
	}
...
export function resolveNativeDeferThresholdRatio(): number {
	const raw = Number(process.env.MEDRESEARCH_NATIVE_DEFER_THRESHOLD_RATIO ?? DEFAULT_NATIVE_DEFER_THRESHOLD_RATIO);
