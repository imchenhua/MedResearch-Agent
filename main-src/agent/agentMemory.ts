...
export function getAgentMemoryDir(
	...
	switch (scope) {
		case 'user':
			return path.join(getCachedMedResearchDataDir(), 'agent-memory', dirName) + path.sep;
		case 'project':
			return workspaceRoot ? path.join(workspaceRoot, '.medresearch', 'agent-memory', dirName) + path.sep : null;
		case 'local':
			return workspaceRoot ? path.join(workspaceRoot, '.medresearch', 'agent-memory-local', dirName) + path.sep : null;
	}
}
