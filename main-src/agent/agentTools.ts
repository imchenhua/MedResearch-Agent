...
export const AGENT_TOOLS: AgentToolDef[] = [
	...
	},
	{
		name: 'Write',
		description:
			'Create a new file or completely overwrite an existing file. This tool writes through MedResearch Agent\'s encoding-safe file path: existing text file encodings/BOMs are preserved when possible, and new files default to UTF-8. For streaming UI, emit the **file_path** argument first, before **content**, so the editor can show the file card title before code starts arriving. For small targeted edits on existing files, prefer **Edit**. When asked to persist MedResearch Agent/Cursor-style project rules as `.mdc` files, use `.medresearch/rules/` under the workspace unless the user specifies another path.',
		parameters: {
			type: 'object',
			properties: {
				file_path: {
		...
		parameters: {
			...
			properties: {
				...
				file_path: {
					type: 'string',
					description:
						'For screenshot_page: optional output path. Workspace-relative or absolute inside the workspace. If omitted, the app saves to `.medresearch/browser-captures/` when a workspace is open, otherwise to a temp folder.',
				},
				timeout_ms: {
					type: 'number',
					description:
	...
	},
	{
		name: 'BrowserCapture',
		description:
			'Capture HTTP traffic from MedResearch Agent\'s built-in browser for the current app session. Typical flow: start capture, use the Browser tool to navigate and interact, then list captured requests and inspect a specific request in detail.',
		parameters: {
			type: 'object',
			properties: {
				action: {
		...
		parameters: {
			...
			properties: {
				...
				file_path: {
					type: 'string',
					description:
						'For screenshot: optional output path. If omitted, saves to `.medresearch/pw-captures/` under the workspace.',
				},
				timeout_ms: {
					type: 'number',
