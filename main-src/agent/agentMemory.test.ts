...
describe('agentMemory', () => {
	it('builds project/local/user memory directories', () => {
		...
		expect(getAgentMemoryDir('reviewer', 'project', root)?.replace(/\\/g, '/')).toContain('/.medresearch/agent-memory/reviewer/');
		expect(getAgentMemoryDir('reviewer', 'local', root)?.replace(/\\/g, '/')).toContain('/.medresearch/agent-memory-local/reviewer/');
