...
describe('executeTool file encoding', () => {
	...
	it('preserves GB18030 when Edit modifies an existing legacy-encoded file', async () => {
		const previous = process.env.MEDRESEARCH_LEGACY_TEXT_ENCODING;
		process.env.MEDRESEARCH_LEGACY_TEXT_ENCODING = 'gb18030';
		try {
			...
			if (previous === undefined) {
				delete process.env.MEDRESEARCH_LEGACY_TEXT_ENCODING;
			} else {
				process.env.MEDRESEARCH_LEGACY_TEXT_ENCODING = previous;
			}
		}
	});
});
