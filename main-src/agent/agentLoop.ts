 ...
 * 3. 如果 LLM 只返回文本 → 结束循环
 * 4. 工具循环轮次：默认不限制；可通过 `MEDRESEARCH_AGENT_MAX_ROUNDS` 或 `settings.agent.maxToolRounds` 设上限。
 */
...
function maxStreamingToolArgChars(): number {
	const raw = process.env.MEDRESEARCH_MAX_STREAMING_TOOL_ARG_CHARS?.trim();
...
function resolveAgentMaxRounds(settings: ShellSettings): number | null {
	const raw = process.env.MEDRESEARCH_AGENT_MAX_ROUNDS?.trim();
