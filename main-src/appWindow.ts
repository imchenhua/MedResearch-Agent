...
const loadDistFlag =
	process.env.MEDRESEARCH_LOAD_DIST === '1' || process.env.VOID_SHELL_LOAD_DIST === '1';
const openDevTools =
	process.env.MEDRESEARCH_DEVTOOLS === '1' || process.env.VOID_SHELL_DEVTOOLS === '1';
...
export function createAppWindow(opts?: {
	...
	const notifyLayout = () => {
		if (!win.isDestroyed()) {
			win.webContents.send('medresearch:layout');
		}
	};
