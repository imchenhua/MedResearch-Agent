...
/**
 * 合并 LSP 来源：插件目录 + 作用域名；MedResearch Agent 另含可选的 TSLS 路径探测与 settings 迁移。
...
export async function getAllLspServers(opts: GetAllLspServersOptions): Promise<Record<string, ScopedLspServerConfig>> {
	...
	const asyncDataPlugins = resolveUserPluginsRoot(opts.settings) || path.join(getCachedMedResearchDataDir(), 'plugins');
	...
	const wsPlugins = path.join(ws, '.medresearch', 'plugins');
