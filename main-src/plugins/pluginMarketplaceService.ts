...
import { getCachedMedResearchDataDir } from '../dataDir.js';
...
function getMarketplaceCacheRoot(): string {
	const root = path.join(getCachedMedResearchDataDir(), 'plugin-marketplaces');
...
function getProjectPluginsRoot(workspaceRoot: string | null | undefined): string | null {
	...
	const root = path.join(path.resolve(String(workspaceRoot)), '.medresearch', 'plugins');
	fs.mkdirSync(root, { recursive: true });
	return root;
}
...
async function materializePluginSourceToTemp(
	...
	try {
		...
		switch (sourceObj.source) {
			...
			case 'npm':
			case 'pip':
				throw new Error(`Plugin source "${sourceObj.source}" is not supported by MedResearch Agent yet.`);
			default:
...
function writeInstallMeta(targetDir: string, meta: AsyncPluginInstallMeta): void {
	writeJsonAtomic(path.join(targetDir, MEDRESEARCH_PLUGIN_META_FILE), meta);
}
