...
const MEDRESEARCH_CA_FRIENDLY_NAME = 'MedResearch Agent Local Capture Root';
const MEDRESEARCH_LINUX_DEST = '/usr/local/share/ca-certificates/medresearch-agent-capture-ca.crt';
...
async function sudoExec(cmd: string): Promise<void> {
	...
	await new Promise<void>((resolve, reject) => {
		sudo.exec(cmd, { name: 'MedResearch Agent' }, (err?: Error, _stdout?: string | Buffer, stderr?: string | Buffer) => {
			if (err) {
...
async function isNamedWindowsCertInStore(scope: CaInstallScope): Promise<boolean> {
	const result = await execCapture(
		`certutil ${scope === 'user' ? '-user ' : ''}-store Root ${quoteCmdArg(MEDRESEARCH_CA_FRIENDLY_NAME)}`.replace(/\s+/g, ' ')
	);
	return result.stdout.includes(MEDRESEARCH_CA_FRIENDLY_NAME);
}
...
async function uninstallWindows(certPath: string | undefined, scope: CaInstallScope): Promise<void> {
	const certId = readPemSha1(certPath) ?? quoteCmdArg(MEDRESEARCH_CA_FRIENDLY_NAME);
	if (scope === 'user') {
...
async function isInstalledMacOS(): Promise<boolean> {
	try {
		const out = await execPromise(
			`security find-certificate -c "${MEDRESEARCH_CA_FRIENDLY_NAME}" /Library/Keychains/System.keychain`
		);
		return out.includes(MEDRESEARCH_CA_FRIENDLY_NAME);
	} catch {
		// Fallback to user keychain.
		try {
			const out = await execPromise(`security find-certificate -c "${MEDRESEARCH_CA_FRIENDLY_NAME}"`);
			return out.includes(MEDRESEARCH_CA_FRIENDLY_NAME);
		} catch {
			return false;
		}
...
async function isInstalledLinux(): Promise<boolean> {
	return existsSync(MEDRESEARCH_LINUX_DEST);
}
...
async function installLinux(certPath: string, _scope: CaInstallScope): Promise<void> {
	...
	await sudoExec(`cp "${certPath}" "${MEDRESEARCH_LINUX_DEST}" && update-ca-certificates`);
}

async function uninstallLinux(_scope: CaInstallScope): Promise<void> {
	if (!existsSync(MEDRESEARCH_LINUX_DEST)) {
		return;
	}
	await sudoExec(`rm -f "${MEDRESEARCH_LINUX_DEST}" && update-ca-certificates --fresh`);
}
