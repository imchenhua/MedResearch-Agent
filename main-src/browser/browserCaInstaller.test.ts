...
const certPath = 'C:\\Users\\Tester\\AppData\\Roaming\\MedResearch Agent\\capture-certificates\\async-capture-ca.pem';
...
describe('CaInstaller on Windows', () => {
	...
	it('falls back to a correctly ordered friendly-name query when the CA file is unavailable', async () => {
		...
		mocks.exec.mockImplementation((cmd: string, _options: unknown, cb: ExecCallback) => {
			if (cmd === 'certutil -user -store Root "MedResearch Agent Local Capture Root"') {
				cb(null, 'Subject: CN=MedResearch Agent Local Capture Root', '');
				return {};
			}
			cb(notFoundError(), '', 'not found');
			return {};
		...
		expect(installed).toBe(true);
		expect(mocks.exec.mock.calls.map(([cmd]) => cmd)).toEqual([
			'certutil -user -store Root "MedResearch Agent Local Capture Root"',
		]);
	});
