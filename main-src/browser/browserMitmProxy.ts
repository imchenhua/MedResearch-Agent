...
class BrowserCaptureCaStore {
	...
	private generateCa(): void {
		...
		const attrs: forge.pki.CertificateField[] = [
			{ shortName: 'CN', value: 'MedResearch Agent Local Capture Root' },
			{ shortName: 'O', value: 'MedResearch Agent' },
		];
...
class BrowserCaptureMitmProxyService {
	...
	private handleConnect(req: http.IncomingMessage, clientSocket: net.Socket, head: Buffer): void {
		...
		clientSocket.write('HTTP/1.1 200 Connection Established\r\nProxy-Agent: MedResearch Agent Capture\r\n\r\n');
	...
	private handleProxyRequest(
		...
		if (!target) {
			...
			clientRes.end('MedResearch Agent proxy could not resolve the target URL.');
			return;
		}
		...
		upstreamReq.on('error', (error) => {
			...
			if (!clientRes.headersSent) {
				...
				clientRes.end('MedResearch Agent proxy upstream request failed.');
			} else {
				clientRes.destroy();
			}
		});
...
function buildUpstreamHeaders(headers: http.IncomingHttpHeaders, hostHeader: string): http.OutgoingHttpHeaders {
	...
	out.host = hostHeader;
	out.via = appendHeaderValue(headers.via, 'MedResearch Agent Capture');
	return out;
}
