...
setWorkspaceFileIndexReadyBroadcaster((rootNorm) => {
	for (const w of BrowserWindow.getAllWindows()) {
		...
		try {
			w.webContents.send('medresearch:workspaceFileIndexReady', rootNorm);
		} catch {
			/* ignore */
		}
	}
...
function readPlanFileForExecute(absPath: string, windowWorkspaceRoot: string | null): string | null {
	...
	const userPlansDir = path.join(app.getPath('userData'), '.medresearch', 'plans');
	const root = windowWorkspaceRoot;
	const wsPlansDir = root ? path.join(root, '.medresearch', 'plans') : null;
	const allowed =
...
export function registerIpc(): void {
	...
	setWorkspaceFsTouchNotifier(() => {
		for (const win of BrowserWindow.getAllWindows()) {
			if (!win.isDestroyed()) {
				win.webContents.send('medresearch:workspaceFsTouched');
			}
		}
	});
	...
	async function persistComposerAttachmentBuffer(
		...
		const dirRel = '.medresearch/composer-drops';
	...
	/** 把工作区 `.medresearch/` 整个移动到 `.medresearch.bak-<ts>/`，用于黑屏/启动异常时的应急恢复。
	...
	ipcMain.handle('workspaceAgent:resetAsyncDir', (event) => {
		...
		const dir = path.join(root, '.medresearch');
		try {
			if (!fs.existsSync(dir)) {
				return { ok: true as const, backupPath: null };
			}
			const backupPath = path.join(root, `.medresearch.bak-${Date.now()}`);
	...
	ipcMain.handle('workspace:deleteSkillFromDisk', (event, skillMdRel: string) => {
		...
		if (
			...
			parts[1] !== 'skills' ||
			!rootSeg ||
			!['.cursor', '.claude', '.medresearch'].includes(rootSeg) ||
			!parts[2] ||
			parts[2].includes('..')
		) {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				clearManagedAgentsForThread({
					threadId,
					emit: (evt) => event.sender.send('medresearch:chat', evt),
				});
	...
	ipcMain.handle('agent:getSession', (event, threadId: string) => {
		...
		if (session) {
			attachManagedAgentEmitter(String(threadId ?? '').trim(), (evt) => {
				event.sender.send('medresearch:chat', evt);
			});
		}
	...
	ipcMain.handle(
		...
		(event, payload: { filename: string; content: string }) => {
			try {
				...
				if (wsRoot) {
					const dir = path.join(wsRoot, '.medresearch', 'plans');
					fs.mkdirSync(dir, { recursive: true });
					const full = path.join(dir, safe);
					fs.writeFileSync(full, content, 'utf8');
					const relPath = path.join('.medresearch', 'plans', safe).replace(/\\/g, '/');
					return { ok: true as const, path: full, relPath };
				}
				const dir = path.join(app.getPath('userData'), '.medresearch', 'plans');
				fs.mkdirSync(dir, { recursive: true });
				const full = path.join(dir, safe);
				fs.writeFileSync(full, content, 'utf8');
