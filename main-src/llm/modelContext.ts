 ...
 *   对应 `claude-code/src/utils/model/modelCapabilities.ts`（MedResearch Agent 侧面向 OpenAI 兼容 `GET /v1/models`）
 *
...
/** 对齐 CC `is1mContextDisabled`（env：`CLAUDE_CODE_DISABLE_1M_CONTEXT` → MedResearch Agent 前缀） */
export function is1mContextDisabled(): boolean {
	return isEnvTruthy(process.env.MEDRESEARCH_DISABLE_1M_CONTEXT);
}
...
function capabilitiesCachePath(): string | null {
	try {
		return join(app.getPath('userData'), '.medresearch', 'cache', 'model-capabilities.json');
	} catch {
		return null;
	}
}
...
/**
 * 对齐 CC `getContextWindowForModel` 的解析顺序（按 MedResearch Agent 环境裁剪）：
 * 1. `MEDRESEARCH_MAX_CONTEXT_TOKENS` 全局覆盖
 * 2. 模型名 `[1m]` → 1M
...
export function getContextWindowForModel(model: string, opts?: ModelContextResolveOpts): number {
	const envMax = process.env.MEDRESEARCH_MAX_CONTEXT_TOKENS?.trim();
	if (envMax) {
 ...
 * 并支持 `MEDRESEARCH_CODE_AUTO_COMPACT_WINDOW` 对窗口再 cap（对齐 `CLAUDE_CODE_AUTO_COMPACT_WINDOW`）。
 */
export function getEffectiveContextWindowSizeForCompress(
	...
	const autoCompactWindow = process.env.MEDRESEARCH_CODE_AUTO_COMPACT_WINDOW?.trim();
 ...
 * 支持 `MEDRESEARCH_AUTOCOMPACT_PCT_OVERRIDE`（对齐 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`）。
...
export function getAutoCompactThresholdForSend(
	...
	const envPercent = process.env.MEDRESEARCH_AUTOCOMPACT_PCT_OVERRIDE?.trim();
	if (envPercent) {
