 ...
 * - `.medresearch/skills/<slug>/SKILL.md`（本应用约定）
 * 与设置里 Skills 合并时按 slug；**优先级：`.medresearch` > `.cursor` > `.claude`**（后者可被前者覆盖）。
 */
export function loadClaudeWorkspaceSkills(workspaceRoot: string | null): AgentSkill[] {
	...
	const medresearchShell = scanSkillsDirectory(workspaceRoot, ['.medresearch', 'skills'], 'medresearch');
	return [...claude, ...cursor, ...medresearchShell];
}
 ...
 * 从全局目录加载磁盘技能（用户主目录级别）：
 * - `~/.claude/skills/<slug>/SKILL.md`
 * - `~/.medresearch/skills/<slug>/SKILL.md`
 * 优先级：`.medresearch` > `.claude`
 */
export function loadGlobalSkills(): AgentSkill[] {
	...
	const medresearchShell = scanSkillsDirectory(home, ['.medresearch', 'skills'], 'medresearch');
	// 标记为全局来源
	...
	return [...markGlobal(claude, 'claude'), ...markGlobal(medresearchShell, 'medresearch')];
}
...
export function loadClaudeWorkspaceSubagents(workspaceRoot: string | null): AgentSubagent[] {
	...
	const medresearchShell = scanSubagentsDirectory(workspaceRoot, ['.medresearch', 'agents'], 'medresearch');
	return [...claude, ...cursor, ...medresearchShell];
}
...
/** 工作区磁盘规则目录：优先 MedResearch Agent 约定，其次 Cursor 兼容路径 */
const THIRD_PARTY_RULE_DIRS = [
	{ segments: ['.medresearch', 'rules'] as const, prefix: '.medresearch/rules' },
...
/** 读取工作区 `.medresearch/rules` 与 `.cursor/rules` 下 .md / .mdc（MedResearch Agent 优先，其次 Cursor 习惯） */
...
export function buildAgentSystemAppend(opts: {
	...
	if (opts.thirdPartyRules.trim()) {
		parts.push(`#### 从项目导入的规则（.medresearch/rules、.cursor/rules、CLAUDE.md、.claude/rules）\n${opts.thirdPartyRules.trim()}`);
	}
