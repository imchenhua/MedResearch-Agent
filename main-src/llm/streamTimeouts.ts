 ...
 * 可通过 MEDRESEARCH_AGENT_STREAM_WATCHDOG=0 关闭静默 watchdog。
 */
...
export function resolveStreamTimeouts(settings: ShellSettings): StreamTimeoutConfig {
	const w = process.env.MEDRESEARCH_AGENT_STREAM_WATCHDOG?.trim().toLowerCase();
	...
	const idleEnv = parseInt(process.env.MEDRESEARCH_AGENT_STREAM_IDLE_MS || '', 10);
	const idleMs =
	...
	const hardEnv = parseInt(process.env.MEDRESEARCH_AGENT_ROUND_HARD_MS || '', 10);
	const hardMsRaw =
