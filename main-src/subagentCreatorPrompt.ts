...
export function buildSubagentCreatorSystemAppend(
	...
	const scopeBlock =
		scope === 'project'
			? lang === 'en'
				? `**Target: this project.** Prefer adding the subagent to workspace **.medresearch/agent.json** or project-scoped agent settings in MedResearch Agent. Workspace root: \`${workspaceRoot ?? '(none)'}\`.`
				: `**目标：本项目。** 优先把 Subagent 写入工作区 **.medresearch/agent.json**，或写入 MedResearch Agent 的项目级 Agent 设置。工作区根目录：\`${workspaceRoot ?? '（无）'}\`。`
			: lang === 'en'
				? '**Target: all projects (user-level).** Describe adding the subagent via MedResearch Agent **Settings → Agent → Subagents** for global use.'
				: '**目标：所有项目（用户级）。** 说明如何通过 MedResearch Agent **设置 → Agent → Subagents** 添加全局 Subagent。';

	const toolBlock =
		lang === 'en'
			? `**Execution mode:** This turn runs in **Agent** with \`Write\` and \`Edit\`.
- If a workspace is open, you **must** persist the subagent by editing project files—typically merge into \`.medresearch/agent.json\` \`subagents\` (or the project's agent JSON MedResearch Agent uses). Do **not** only paste JSON for the user to copy; use tools, then confirm paths.
...
- 只要工作区已打开，就**必须**用工具把 Subagent 持久化到项目文件中，通常是合并进 \`.medresearch/agent.json\` 的 \`subagents\`（或 MedResearch Agent 实际使用的项目级 agent JSON）。**不要**只贴一段 JSON 让用户自己复制；应直接改文件，并说明写入位置。
- 如果是**用户级 / 所有项目**，但当前没有工作区，工具无法写入应用 userData。请明确说明这一点，只给出最小必要的手动添加步骤，不要声称已经写入。
- **本项目**范围必须有工作区。`;

	const core =
		lang === 'en'
			? `You are MedResearch Agent's **Subagent Creator**. The user's notes appear after the scope tag.
...
4. End with a short note on how it takes effect in MedResearch Agent for the chosen scope.

${scopeBlock}`
			: `你是 MedResearch Agent 的 **Subagent 创建向导**。用户说明会出现在范围标签之后。
...
4. 最后用简短文字说明：它会在 MedResearch Agent 中以什么范围生效。

${scopeBlock}`;

	return `### MedResearch Agent · Subagent Creator（内置）\n\n${core}`;
}
