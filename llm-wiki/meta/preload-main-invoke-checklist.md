...
Renderer 通过 `window.medresearchShell.invoke(channel, ...args)` 发 IPC；preload 在 invoke 前执行：
...
5. 在 renderer 实际路径上手动点一次或跑相关 E2E，确认不再出现 `medresearch: blocked IPC channel`。
