...
- 状态：这是 `llm-wiki` 的维护规则，而不是产品功能说明。
...
3. 当前测试
4. README / docs
5. `.medresearch/memory`
6. `.medresearch/index/` 等运行时生成索引

规则：

- 上层与下层冲突时，不要“平均采信”，而要写清楚谁是当前事实、谁是旧信息。
- `.medresearch/memory` 可以提供线索，但不能跳过代码核验。
...
3. 如果旧信息来自 `.medresearch/memory`，考虑同步刷新或删除对应记忆。

## 关于 `.medresearch/memory`

建议把它当作“面向对话注入的压缩记忆”，而不是文档系统本身。

推荐分工：

- `llm-wiki/`：架构、事实、规则、冲突、导航
- `.medresearch/memory/`：短索引、长期偏好、便于自动加载的记忆钩子

注意：

- 当前仓库默认在 `.gitignore` 中忽略 `.medresearch/`
- 因此 `.medresearch/agent.json`、`.medresearch/memory/`、`.medresearch/index/` 默认都偏本地运行时资产
- 真正需要团队共享的知识，应优先编译到 `llm-wiki/`

如果某条 `.medresearch/memory` 记忆已经明显过时，应当：
...
- 新说法是否和源码一致
- 有无新的冲突需要记录
- 是否需要同步 `.medresearch/agent.json` 的入口规则
