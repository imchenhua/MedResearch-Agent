...
- 主题：MedResearch Agent 把什么状态存在哪里，以及 `docs Wiki`、`.medresearch/memory`、线程/计划之间的关系。

## 持久化版图

### 1. 全局用户数据目录

由 `resolveMedResearchDataDir(userData)` 统一解析到 `userData/medresearch/`：
...
### 2. 工作区目录下的 `.medresearch/`

当前仓库会额外持有这些工作区级数据：

- `.medresearch/agent.json`：项目级 Agent rules / skills / subagents
- `.medresearch/plans/*.md`：当工作区已打开时，Plan 文档优先写到这里
- `.medresearch/memory/`：项目级持久记忆
- `.medresearch/agent-memory/`：项目级子 Agent 记忆
- `.medresearch/agent-memory-local/`：本机局部子 Agent 记忆
- `.medresearch/index/`：运行时索引产物
...
- Markdown 文档：`ipc register -> plan:save`
  - 有工作区时写到 `<workspace>/.medresearch/plans/`
  - 无工作区时回退到 `userData/.medresearch/plans/`
- 结构化 plan：`threads.json` 的 `thread.plan`

这意味着：

- 只看 README 里“`.medresearch/plans/`”的描述不够完整。
- 做 Plan 相关改动时，要区分“给人看的 Markdown 文档”和“线程内结构化状态”。
- 渲染层如何把流式输出、消息历史与上述双写串起来，见 [usePlanSystem.ts](../modules/use-plan-system.md)。

## 项目级 Agent 配置

`workspaceAgentStore.ts` 负责 `<workspace>/.medresearch/agent.json`：

- 它只存项目级 rules / skills / subagents
- 读出来后与用户全局 `settings.agent` 合并
- 这是把“仓库内的知识入口规则”注入给 Agent 的关键位置
- 但当前仓库的 `.gitignore` 默认忽略整个 `.medresearch/`，所以这类配置默认是本机本仓库有效，而不是天然团队共享

## 项目级记忆：`.medresearch/memory/`
...
`extractMemories.ts` 会基于最近对话，调用模型生成新的记忆草稿并回写到 `.medresearch/memory/`。
...
它们与项目级 `.medresearch/memory/` 不同，不应混为一类。
...
- 结构化、长期有效、需要交叉链接的知识放 `llm-wiki/`
- 对话上下文导向、短索引型记忆放 `.medresearch/memory/`
