...
| `workspace:saveComposerAttachment` | Composer 拖拽附件落盘到 `.medresearch/composer-drops` |
...
| `workspaceAgent:get` / `workspaceAgent:set` / `workspaceAgent:resetAsyncDir` | `.medresearch/agent.json` 项目切片与重置 |
...
| `plan:save` | Markdown Plan 落盘（工作区或 userData 下 `.medresearch/plans`） |
...
流式聊天、布局、终端数据等多半通过 `webContents.send` / preload `subscribe*` 下发；具体频道以 `preload.cjs` 与 `register.ts` 内 `send('medresearch:…')` 为准，本页不逐项罗列。
