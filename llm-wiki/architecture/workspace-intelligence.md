...
- 跳过 `.git`、`node_modules`、`dist`、`.idea`、`.medresearch/index/`、`.medresearch/memory/` 等目录
- 有最大文件数限制：`MAX_WORKSPACE_FILES = 5000`
- 会维护排序快照和 basename bucket，以支持更快搜索

它不是单纯的“列目录”，而是 MedResearch Agent 工作区智能的第一层基础设施。
...
- `.medresearch/index/semantic.json` 仍残留了这个文件的索引内容（运行时历史残影）
