...
- `project` -> `<workspace>/.medresearch/agent-memory/<agent>/`
- `local` -> `<workspace>/.medresearch/agent-memory-local/<agent>/`

## 工作区级 Agent 配置

`main-src/workspaceAgentStore.ts` 说明：

- 当前仓库可通过 `<workspace>/.medresearch/agent.json` 注入项目级 `rules / skills / subagents`
