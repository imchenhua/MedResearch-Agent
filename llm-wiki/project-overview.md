...
- 主题：MedResearch Agent 作为产品的定位、边界和当前已实现能力。

## 一句话定义

MedResearch Agent 是一个开源、桌面端、Agent-first 的 AI IDE Shell。它不是“给现有编辑器塞一个聊天侧栏”，而是把 Agent、编辑器、Git、终端、计划审阅、模型配置和工具执行放进同一个桌面工作区。
...
- 运行时记忆体系分层明显：`llm-wiki` 不是 `.medresearch/memory` 的替代，而是更可审阅的上层知识层。
