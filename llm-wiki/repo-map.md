...
| `dist/` | 渲染层构建产物 | 生成目录 |
| `.medresearch/` | 工作区运行时数据 | 记忆、索引、Agent 项目切片等 |
...
| `main-src/memdir/` | `.medresearch/memory` 入口、扫描、类型定义 |
...
- `dist/`：由 Vite 生成。
- `.medresearch/index/`：运行时索引结果，可能包含历史条目，不应直接当源码真相。
- `.medresearch/memory/`：项目运行时记忆，不等同于已审阅文档。
