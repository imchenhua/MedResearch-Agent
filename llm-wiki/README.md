...
- 代码优先：当 Wiki、README、`.medresearch/memory`、生成索引互相冲突时，以当前源码为准。
...
- 想查线程、设置、计划、`.medresearch/memory`、工作区级 AI 配置：看 [状态与记忆](./architecture/state-and-memory.md)。
...
- `llm-wiki/`：人工维护、可审阅、偏稳定的知识层，适合放架构、事实、规则、冲突。
- `.medresearch/memory/`：运行时记忆入口，适合放更短的持久记忆；它会影响对话上下文，但不天然等于“权威文档”。
- `.medresearch/agent.json`：项目级 Agent 规则、技能和子 Agent 配置，用来告诉 AI 先看什么、如何行动；但当前仓库默认忽略 `.medresearch/`，所以它是本机级配置，未必会随 Git 共享。
- `README*.md`：对外说明文档，适合介绍项目，但不保证比代码更及时。
- `.medresearch/index/`：运行时索引产物，便于检索，但可能保留历史残影，不能直接当事实来源。
...
3. 把现有 `.medresearch/memory` 里的有效信息重新放回代码语境里校验。
...
- Phase 6：把每次大功能开发后的结论编译进专题页，而不是只落在聊天记录或 `.medresearch/memory` 里。
