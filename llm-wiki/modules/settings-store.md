...
`settingsStore.ts` 定义并持久化 MedResearch Agent 的全局设置模型，是模型、Agent、UI、MCP、Team、bot 和统计配置的总入口。
...
1. 定位到 `userData/medresearch/settings.json`
2. 读取已有设置
3. 执行若干迁移
4. 如有需要则回写
...
- `<workspace>/.medresearch/agent.json`
