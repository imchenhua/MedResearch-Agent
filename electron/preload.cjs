...
ipcRenderer.on('medresearch:chat', (_event, payload) => {
...
ipcRenderer.on('medresearch:layout', () => {
...
ipcRenderer.on('medresearch:themeMode', (_event, payload) => {
...
ipcRenderer.on('medresearch:workspaceFsTouched', () => {
...
ipcRenderer.on('medresearch:workspaceFileIndexReady', (_event, rootNorm) => {
...
ipcRenderer.on('medresearch:pluginsChanged', () => {
...
ipcRenderer.on('medresearch:browserNewWindow', (_event, payload) => {
...
ipcRenderer.on('medresearch:googleLoginExternal', (_event, payload) => {
...
ipcRenderer.on('medresearch:browserControl', (_event, payload) => {
...
ipcRenderer.on('medresearch:openSettingsNav', (_event, nav) => {
...
ipcRenderer.on('medresearch:composerAppendDraft', (_event, payload) => {
...
ipcRenderer.on('medresearch:captureAnalysisDispatch', (_event, payload) => {
...
ipcRenderer.on('medresearch:trayCommand', (_event, payload) => {
...
contextBridge.exposeInMainWorld('medresearchShell', {
	invoke(channel, ...args) {
		if (!INVOKE_CHANNELS.has(channel)) {
			throw new Error(`medresearch: blocked IPC channel "${channel}"`);
		}
