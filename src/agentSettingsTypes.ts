...
export type AgentSkill = {
	...
	name: string;
	description: string;
	/** 技能标识；也可配合仓库 `.medresearch/skills/<slug>/SKILL.md` */
	slug: string;
	content: string;
	enabled?: boolean;
	origin?: AgentItemOrigin;
...
export type AgentCustomization = {
	/** 从工作区 `.medresearch/rules`、`.cursor/rules` 等目录导入规则文本 */
...
/** 主进程从 `.claude` / `.cursor` / `.medresearch` 的 skills 目录扫描出的项，id 形如 `ws-skill-*`；不应写入 settings 或 `.medresearch/agent.json`。 */
export function isWorkspaceDiskImportedSkill(s: { id: string }): boolean {
	return s.id.startsWith('ws-skill-');
}

/** 从用户主目录 `~/.claude/skills/`、`~/.medresearch/skills/` 扫描出的全局 skill，id 形如 `global-skill-*`；不应写入 settings 或 `.medresearch/agent.json`。 */
