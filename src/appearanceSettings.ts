...
export type ThemePresetId = 'medresearch' | 'cursor' | 'graphite' | 'forest' | 'sunset';
...
export const THEME_PRESET_IDS: readonly ThemePresetId[] = ['medresearch', 'cursor', 'graphite', 'forest', 'sunset'];
...
export const APPEARANCE_THEME_PRESETS: Record<ThemePresetId, Record<'light' | 'dark', AppearanceChromeSeed>> = {
	medresearch: BUILTIN_COLOR_SCHEME_APPEARANCE,
...
/**
 * mac-codex 布局大量依赖 --surface-* / --shadow-*；若只改 --void-bg-* 而不覆盖这些，换主题时侧栏/卡片仍像默认 MedResearch Agent。
 */
