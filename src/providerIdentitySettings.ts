...
export type ResolvedProviderIdentitySettings = {
	...
	systemPromptPrefix: string;
	anthropicMetadataMode: 'medresearch' | 'claude-code';
...
const MEDRESEARCH_DEFAULT_IDENTITY: ResolvedProviderIdentitySettings = {
	preset: 'medresearch-default',
	userAgentMode: 'generic',
	userAgentProduct: 'medresearch-agent',
	entrypoint: 'desktop',
	appHeaderValue: 'desktop',
	clientAppValue: 'medresearch-agent',
	sessionHeaderName: 'X-MedResearch-Session-Id',
	systemPromptPrefix: 'You are MedResearch Agent, a research assistant for clinical and medical research.',
	anthropicMetadataMode: 'medresearch',
};
...
function inferPreset(raw?: ProviderIdentitySettings | null): ProviderIdentityPreset {
	...
	const legacyValues = {
		userAgentProduct: cleanToken(raw?.userAgentProduct, MEDRESEARCH_DEFAULT_IDENTITY.userAgentProduct),
		entrypoint: cleanToken(raw?.entrypoint, MEDRESEARCH_DEFAULT_IDENTITY.entrypoint),
		appHeaderValue: cleanToken(raw?.appHeaderValue, MEDRESEARCH_DEFAULT_IDENTITY.appHeaderValue),
		clientAppValue: cleanToken(raw?.clientAppValue, MEDRESEARCH_DEFAULT_IDENTITY.clientAppValue),
		systemPromptPrefix: cleanToken(raw?.systemPromptPrefix, MEDRESEARCH_DEFAULT_IDENTITY.systemPromptPrefix),
	};
	const matchesAsyncDefault =
		legacyValues.userAgentProduct === MEDRESEARCH_DEFAULT_IDENTITY.userAgentProduct &&
		legacyValues.entrypoint === MEDRESEARCH_DEFAULT_IDENTITY.entrypoint &&
		legacyValues.appHeaderValue === MEDRESEARCH_DEFAULT_IDENTITY.appHeaderValue &&
		legacyValues.clientAppValue === MEDRESEARCH_DEFAULT_IDENTITY.clientAppValue &&
		legacyValues.systemPromptPrefix === MEDRESEARCH_DEFAULT_IDENTITY.systemPromptPrefix;
	if (matchesAsyncDefault) {
		return 'medresearch-default';
	}
...
export function resolveProviderIdentitySettings(
	...
	if (preset === 'custom') {
		return {
			...
			appHeaderValue: cleanToken(raw?.appHeaderValue, MEDRESEARCH_DEFAULT_IDENTITY.appHeaderValue),
			clientAppValue: cleanToken(raw?.clientAppValue, MEDRESEARCH_DEFAULT_IDENTITY.clientAppValue),
			sessionHeaderName: MEDRESEARCH_DEFAULT_IDENTITY.sessionHeaderName,
			systemPromptPrefix: cleanToken(raw?.systemPromptPrefix, MEDRESEARCH_DEFAULT_IDENTITY.systemPromptPrefix),
			anthropicMetadataMode: 'medresearch',
		};
	}
	return { ...MEDRESEARCH_DEFAULT_IDENTITY };
}
...
export function providerIdentityPresetOptions(): Array<{
	...
	return [
		{ id: 'medresearch-default', userAgentMode: 'generic', label: 'MedResearch Agent' },
