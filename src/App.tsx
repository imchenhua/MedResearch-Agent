...
function useMedResearchShell() {
	return window.medresearchShell;
}
...
export default function App({
	...
	const shell = useMedResearchShell();
...
function AppMainWorkspaceInner() {
	...
	useEffect(() => {
		const shellApi = window.medresearchShell;
		if (!shellApi) {
			return;
		}
	...
	useEffect(() => {
		const unsub = window.medresearchShell?.subscribeOpenSettingsNav?.((nav) => {
	...
	useEffect(() => {
		const unsub = window.medresearchShell?.subscribeTrayCommand?.((payload) => {
	...
	useEffect(() => {
		const unsub = window.medresearchShell?.subscribeComposerAppendDraft?.((payload) => {
			const rawText =
	...
	useEffect(() => {
		...
		window.addEventListener('medresearch:focusThread', handler);
		return () => {
			window.removeEventListener('medresearch:focusThread', handler);
		};
	}, [refreshThreads, setCurrentId, loadMessages]);

	useEffect(() => {
		const unsub = window.medresearchShell?.subscribeCaptureAnalysisDispatch?.((payload) => {
	...
	useEffect(() => {
		...
		const unsubLayout = window.medresearchShell?.subscribeLayout?.(onResize);
		return () => {
