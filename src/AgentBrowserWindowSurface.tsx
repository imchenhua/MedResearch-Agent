...
function safeGetWebviewUrl(node: MedResearchShellWebviewElement | null): string {
	if (!node) {
		return '';
	}
	try {
...
function browserCaptureBuildJsonExport(
	...
	return JSON.stringify(
		{
			version: 1,
			source: 'MedResearch Agent browser capture',
	...
	return JSON.stringify(
		{
			log: {
				version: '1.2',
				creator: {
					name: 'MedResearch Agent browser capture',
					version: '1.0',
				},
				pages: [],
...
const BrowserTabView = memo(
	function BrowserTabView({
		...
		onRegisterWebview: (id: string, node: MedResearchShellWebviewElement | null) => void;
	}) {
	const webviewRef = useRef<MedResearchShellWebviewElement | null>(null);
	...
	const assignWebviewRef = useCallback(
		(node: MedResearchShellWebviewElement | null) => {
...
const AgentRightSidebarBrowserPanel = memo(function AgentRightSidebarBrowserPanel({
	...
	const webviewsRef = useRef<Map<string, MedResearchShellWebviewElement>>(new Map());
	...
	const installBrowserCaptureProxyCa = useCallback(
		async (uninstall: boolean = false, scope: 'user' | 'machine' = 'user') => {
			...
			const fallbackMsg = uninstall
				? 'Remove the MedResearch Agent capture root certificate from the trust store?'
				: scope === 'machine'
					? 'Install the MedResearch Agent capture root CA system-wide? This requires administrator rights.'
					: 'Install the MedResearch Agent capture root CA into your user trust store? Windows/macOS will ask you to confirm.';
			const message = (() => {
	...
	const waitForWebviewNode = useCallback((tabId: string, timeoutMs: number = 10_000): Promise<MedResearchShellWebviewElement> => {
	...
	const waitForWebviewSettled = useCallback(
		(node: MedResearchShellWebviewElement, tabId: string, timeoutMs: number = 15_000): Promise<void> => {
	...
	const readPageFromWebview = useCallback(
		async (
			node: MedResearchShellWebviewElement,
	...
	const captureWebviewScreenshot = useCallback(async (node: MedResearchShellWebviewElement): Promise<Record<string, unknown>> => {
		...
		async (record: BrowserCaptureAnalysisRecordUi) => {
			...
			try {
				...
				window.dispatchEvent(
					new CustomEvent('medresearch:focusThread', { detail: { threadId: record.threadId } })
				);
			} catch {
				/* ignore */
			}
	...
	const handleRegisterWebview = useCallback((id: string, node: MedResearchShellWebviewElement | null) => {
		if (node) {
	...
	// receives 'medresearch:browserNewWindow' from web-contents-created hook in main.
	useEffect(() => {
