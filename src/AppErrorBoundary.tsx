...
type MedResearchShellLike = {
	invoke: (channel: string, ...args: unknown[]) => Promise<unknown>;
};

function getMedResearchShell(): MedResearchShellLike | null {
	const w = window as unknown as { medresearchShell?: MedResearchShellLike };
	return w.medresearchShell ?? null;
}

/**
 * 渲染进程根错误兜底：避免初始化时单点异常把整个窗口顶成黑屏。
 * 命中时显示故障页：重载、备份并重置当前工作区 .medresearch/ 目录。
 */
export class AppErrorBoundary extends Component<Props, State> {
	...
	private handleResetAsync = async (): Promise<void> => {
		const shell = getMedResearchShell();
		if (!shell) {
			...
			if (result.ok) {
				this.setState({
					...
					resetMessage: result.backupPath
						...
						: 'No .medresearch folder was found. Reload to continue.',
				});
			} else {
		...
		return (
			<div className="ref-app-error-boundary" role="alert">
				<div className="ref-app-error-boundary-card">
					<h1 className="ref-app-error-boundary-title">MedResearch Agent ran into a problem</h1>
					<p className="ref-app-error-boundary-msg">
						...
						<code>.medresearch/</code> folder (for example <code>.medresearch/agent.json</code>). Try reloading first; if it still
					...
					<div className="ref-app-error-boundary-actions">
						...
						>
							{resetting ? 'Resetting…' : 'Back up & reset .medresearch/'}
						</button>
					</div>
					{resetMessage ? (
						<p
