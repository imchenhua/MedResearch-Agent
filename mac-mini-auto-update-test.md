# MedResearch Agent macOS Auto-Update Test Guide (Mac Mini)

Use this guide to build and test MedResearch Agent's macOS auto-update and differential update features on the Mac mini (`192.168.1.201`).

## Important Note: No Code Signing

MedResearch Agent is **not code-signed** on macOS. This means:
...
| IP | `192.168.1.201` |
| SSH User | `licl` |
| Local project (Windows) | `D:\WebstormProjects\MedResearch-Agent` |
| Remote project | `~/MedResearch-Agent` |
...
cd ~
git clone https://github.com/imchenhua/MedResearch-Agent.git MedResearch-Agent
...
cd D:\WebstormProjects\MedResearch-Agent

# Exclude node_modules and build artifacts
tar -czf medresearch-mac-test.tgz --exclude=node_modules --exclude=release --exclude=dist --exclude=.git .

# Upload to Mac mini
scp medresearch-mac-test.tgz licl@192.168.1.201:/Users/licl/

# Extract on Mac mini
ssh licl@192.168.1.201 "mkdir -p ~/MedResearch-Agent && tar -xzf ~/medresearch-mac-test.tgz -C ~/MedResearch-Agent && rm -f ~/medresearch-mac-test.tgz"
```
...
Output:
- `release/MedResearch-Agent-0.0.28-x64.dmg`
- `release/MedResearch-Agent-0.0.28-x64.zip`
- `release/MedResearch-Agent-0.0.28-arm64.dmg`
- `release/MedResearch-Agent-0.0.28-arm64.zip`
...
unzip -q ~/MedResearch-Agent/release/MedResearch-Agent-0.0.28-arm64.zip -d ~/MedResearch-Agent-Test
```
...
xattr -cr ~/MedResearch-Agent-Test/MedResearch\ Agent.app
```
...
~/MedResearch-Agent-Test/MedResearch\ Agent.app/Contents/MacOS/MedResearch\ Agent
```
...
- Menu: **MedResearch Agent -> About MedResearch Agent**
...
cp ~/MedResearch-Agent/release/latest-mac.yml ./
cp ~/MedResearch-Agent/release/MedResearch-Agent-0.0.28-arm64.zip ./MedResearch-Agent-0.0.29-arm64.zip

# Edit latest-mac.yml:
# 1. Change version to "0.0.29"
# 2. Change path to "MedResearch-Agent-0.0.29-arm64.zip"
...
5. Click **"打开下载文件夹"**.
6. Finder opens `~/Downloads/` showing `MedResearch-Agent-0.0.29-mac-update.zip`.
7. Manually unzip and replace the old app:
   ```bash
   unzip -q ~/Downloads/MedResearch-Agent-0.0.29-mac-update.zip -d ~/MedResearch-Agent-Test-New
   xattr -cr ~/MedResearch-Agent-Test-New/MedResearch\ Agent.app
   ```
...
```bash
ls -la ~/MedResearch-Agent/release/*.blockmap
```

Expected output:
```
MedResearch-Agent-0.0.28-x64.zip.blockmap
MedResearch-Agent-0.0.28-arm64.zip.blockmap
```

Verify `latest-mac.yml` contains `blockMapSize`:

```bash
cat ~/MedResearch-Agent/release/latest-mac.yml | grep blockMapSize
```
...
```bash
cat ~/Library/Logs/MedResearch\ Agent/main.log
tail -f ~/Library/Logs/MedResearch\ Agent/main.log
```
